...
def connectToy(toys_list, toy_names):
	...
	if toy_name in toy_names:
		...
		print('The desired sphero-BOLT was not found')
		exit()
...
class BoltControl(object):
	...
	def spheroBolt_execute_action(self, my_toy):
		...
		if not self.cmd_q.empty():
			cmds = self.cmd_q.get()
			print(f'queue value: \n speed: {cmds[0]} \n angle: {cmds[1]}')
			my_toy.roll(cmds[1], cmds[0], self.duration)
	...
	def sphero_callback(self, _key_msg):  			
		if not self.cmd_q.full():
			self.twist_msg = _key_msg
			self.speed = int(round(self.twist_msg.linear.x))
			
			rospy.loginfo("callback: speed = %d  -- angular-speed: %d", self.speed, int(round(self.twist_msg.angular.z))%360)
			self.angular = self.angular + int(round(self.twist_msg.angular.z))
			self.cmd_q.put([self.speed, self.angular])
		else:
			print(f'cmd queue full')
